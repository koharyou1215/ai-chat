...
// Claudeキャラクターファイルを個別JSONファイルに変換
async function convertClaudeCharacters() {
  console.log('🔄 Claudeキャラクター変換開始...');
  
  try {
    // Claudeキャラクターファイルを読み込み
    const claudeFilePath = path.join(__dirname, 'Claud_utput_character.md');
    ...
    for (let i = 0; i < characterBlocks.length; i++) {
      try {
        ...
        if (characterVariants[baseName] === 1) {
          filename = `${baseName}-claude.json`;
        } else {
          // 2つ目以降は番号を付ける
          filename = `${baseName}（パターン${characterVariants[baseName]}）-claude.json`;
    ...
    console.log(`\n✅ Claude変換完了: ${savedCount}個のキャラクターファイルを保存しました`);
    ...
    Object.keys(characterVariants).forEach(name => {
      ...
      if (count === 1) {
        console.log(`  - ${name}-claude.json`);
      } else {
        for (let i = 1; i <= count; i++) {
          if (i === 1) {
            console.log(`  - ${name}-claude.json`);
          } else {
            console.log(`  - ${name}（パターン${i}）-claude.json`);
          }
        }
      }
    });

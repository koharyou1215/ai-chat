...
import { OPEN_ROUTER_MODELS } from '@/config/model-config';
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Max Tokens (出力): {formSettings.maxTokens}
          </label>
          <input
            type="range"
            min="100"
          ...
          <p className="text-xs text-gray-500 mt-1">
            ⚠️ 入力+出力の合計でトークン消費。履歴40件≈6000トークン + 出力{formSettings.maxTokens}トークン = 約{Math.round((6000 + formSettings.maxTokens) / 1000)}Kトークン/回
          </p>
        </div>

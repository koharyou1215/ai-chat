    ...
    try {
      if (appSettings.voiceProvider === 'voicevox') {
        ...
        console.log('🎵 VOICEVOX音声再生開始:', {
          text: text.substring(0, 50),
          speaker: appSettings.voicevoxSpeaker,
          apiUrl: appSettings.voicevoxApiUrl
        });
      ...
      if (appSettings.voiceProvider === 'voicevox') {
        ...
        // ローカルAPIの場合、接続テストを提案
        if (appSettings.voicevoxApiUrl?.includes('localhost')) {
          console.warn('💡 ローカルVOICEVOX接続に失敗しました。VOICEVOX本体が起動していることを確認してください。');
        }
      }
    } finally {

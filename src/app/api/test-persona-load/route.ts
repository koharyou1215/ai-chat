...
export async function GET() {
  try {
    ...
    // 複数のペルソナディレクトリをチェック
    const possibleDirs = [
      path.join(process.cwd(), 'public', 'personas'),
      path.join(process.cwd(), 'public', 'persona'),
    ...
    console.log('📂 ペルソナディレクトリ状況:', dirStatus);
    ...
    console.error('❌ ペルソナ読み込みテストエラー:', error);
    return NextResponse.json({
      error: error instanceof Error ? error.message : 'Unknown error',
      timestamp: new Date().toISOString()
    }, { status: 500 });
